...
class TestThreadSafety:
    ...
    def test_concurrent_mixed_operations(self, stub_handler):
        ...
        def execute_agent(agent_id):
            ...
            mock_wrapped = mock.MagicMock(return_value=f"Agent result")
            crewai_module._wrap_agent_execute_task(mock_wrapped, mock_agent, (), {"task": mock_task})
    ...
    def test_concurrent_instrumentation_does_not_crash(self, tracer_provider, meter_provider):
        ...
        assert len(errors) == 0, f"Errors occurred during concurrent instrumentation: {errors}"
    ...
    def test_error_handling_in_concurrent_operations(self, stub_handler):
        ...
        num_threads = 5
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
            futures = [executor.submit(execute_failing_workflow, i) for i in range(num_threads)]
            results = [f.result() for f in concurrent.futures.as_completed(futures)]
