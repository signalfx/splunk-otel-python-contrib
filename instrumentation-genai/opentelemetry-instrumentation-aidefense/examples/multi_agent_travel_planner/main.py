...
def create_llm(agent_name: str, temperature: float = 0.5) -> ChatOpenAI:
    ...
    return ChatOpenAI(
        ...
        model_kwargs={"user": json.dumps({"appkey": LLM_APP_KEY})} if LLM_APP_KEY else {},
...
def mock_search_activities(destination: str) -> str:
    ...
    return activities.get(destination.lower(), f"Popular activities in {destination}: City tour, Local cuisine")
...
def activity_specialist_node(state: PlannerState) -> PlannerState:
    ...
    state["activities_summary"] = final_message.content if isinstance(final_message, BaseMessage) else str(final_message)
    state["messages"].append(final_message if isinstance(final_message, BaseMessage) else AIMessage(content=str(final_message)))
...
def main():
    ...
    with tracer.start_as_current_span(
        ...
        if final_state and final_state.get("blocked_by_security"):
            ...
            root_span.set_attribute("travel.security_event_id", final_state["security_event_id"])
    ...
    if final_state and final_state["blocked_by_security"]:
        print("\n🚨 SECURITY ALERT:")
        print(f"   Trip blocked due to harmful content!")
