...
- Made `cisco-aidefense-sdk` fully optional so Gateway Mode works without the SDK installed
- Removed `cisco-aidefense-sdk` from `instruments` optional dependency in `pyproject.toml`
- SDK Mode continues to handle missing SDK gracefully via `try/except ImportError`
