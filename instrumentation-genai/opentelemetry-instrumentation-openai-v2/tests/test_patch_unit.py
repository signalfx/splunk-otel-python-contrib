...
def _make_response(choices):
    return SimpleNamespace(choices=choices, id="resp-1", model="gpt-4o")
...
class TestContentToParts:
    ...
    def test_text_block(self):
        parts = _content_to_parts([{"type": "text", "text": "world"}])
        assert parts == [Text(content="world")]

    def test_reasoning_block(self):
        parts = _content_to_parts([{"type": "reasoning", "content": "step"}])
        assert len(parts) == 1
        assert isinstance(parts[0], Reasoning)
        assert parts[0].content == "step"

    def test_thinking_block(self):
        parts = _content_to_parts([{"type": "thinking", "content": "think"}])
        assert isinstance(parts[0], Reasoning)
        assert parts[0].content == "think"

    def test_unknown_block_becomes_generic_part(self):
        block = {"type": "image_url", "image_url": {"url": "http://x"}}
...
class TestBuildOutputMessagesNewTypes:
    def test_flag_off_tool_call_produces_legacy_type(self, monkeypatch):
        monkeypatch.delenv(
    ...
    def test_flag_on_reasoning_block(self, monkeypatch):
        monkeypatch.setenv(
            "OTEL_INSTRUMENTATION_GENAI_ENABLE_NEW_MESSAGE_TYPES", "true"
        )
    ...
    def test_no_choices_returns_empty(self, monkeypatch):
        ...
        assert _build_output_messages_from_response(_make_response([])) == []
