...
def _content_to_parts(content: Any) -> list[MessagePart]:
    ...
    for item in content:
        ...
        if block_type == "text":
            ...
            parts.append(Text(content=text_val))
            ...
            parts.append(Reasoning(content=str(reasoning_val)))
        else:
            parts.append(GenericPart(value=item))
    return parts


def _build_tool_call_request(tool_call: Any) -> ToolCallRequest:
    ...
    for choice in getattr(result, "choices", []) or []:
        ...
        if tool_calls:
            for tool_call in tool_calls:
                if new_types:
                    parts.append(_build_tool_call_request(tool_call))
                else:
...
def _content_to_parts(
    ...
    Reasoning summary blocks from o-series models become Reasoning parts.
    Unknown block types are wrapped in GenericPart.
    Always populates full content; emitter layer controls what gets written.
    """
    if content is None:
        return []
    if isinstance(content, str):
