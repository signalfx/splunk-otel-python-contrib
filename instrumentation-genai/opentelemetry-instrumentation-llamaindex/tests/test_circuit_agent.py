...
async def test_circuit_agent_attributes(span_exporter, instrument):
    ...
    gen_ai.tool.definitions, gen_ai.request.max_tokens, gen_ai.request.stream,
    gen_ai.response.time_to_first_chunk, and token usage.
    """
    ...
    try:
        ...
        # Streaming attributes (ReActAgent uses streaming)
        assert attrs.get("gen_ai.request.stream") is True
        assert attrs.get("gen_ai.response.time_to_first_chunk") is not None
