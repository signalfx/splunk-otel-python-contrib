...
class TraceValidator:
    ...
    def validate_genai_schema(span: Dict, strict: bool = False) -> bool:
        ...
        if operation:
            assert (
                operation in TraceValidator.VALID_OPERATIONS
        ...
        if strict:
            for attr in TraceValidator.OPTIONAL_GENAI_ATTRIBUTES:
                assert (
                    attr in attributes
                ), f"Missing optional attribute (strict mode): {attr}"

        logger.info(
            "GenAI schema valid", span_id=span.get("span_id"), operation=operation
        )
        return True
    ...
    def validate_parent_child(parent_span: Dict, child_span: Dict) -> bool:
        ...
        assert (
            parent_span_id == child_parent_id
    ...
    def find_spans_by_attribute(
        trace: Dict, attribute_name: str, attribute_value: Any
    ) -> List[Dict]:
        """
    ...
    def validate_span_hierarchy(trace: Dict) -> bool:
        ...
        for span in spans:
            ...
            if parent_id:
                assert (
                    parent_id in span_ids
                ), f"Span {span.get('span_id')} references non-existent parent {parent_id}"

        logger.info(
            "Span hierarchy valid", total_spans=len(spans), root_spans=len(root_spans)
        )
        return True
    ...
    def validate_token_usage(span: Dict) -> bool:
        ...
        logger.info(
            "Token usage valid", input_tokens=input_tokens, output_tokens=output_tokens
        )
        return True
    ...
    def validate_streaming_attributes(span: Dict) -> bool:
        ...
        if is_streaming:
            ...
            assert isinstance(
                ttft, (int, float)
            ), f"TTFT must be numeric, got {type(ttft)}"
    ...
    @staticmethod
    def validate_genai_trace(trace: Dict, expected_system: str = None) -> Dict:
        """
        Validate entire trace for GenAI semantic conventions.

        Args:
            trace: Trace dictionary
            expected_system: Expected gen_ai.system value (e.g., 'openai', 'crewai')

        Returns:
            Validation result dictionary with 'valid' boolean and optional 'errors' list
        """
        errors = []
        genai_spans = []

        # Find all GenAI spans
        for span in trace.get("spans", []):
            attributes = span.get("attributes", {})
        ...
        if not genai_spans:
            errors.append("No GenAI spans found in trace")
            return {"valid": False, "errors": errors}

        # Validate each GenAI span
        for span in genai_spans:
            ...
            for attr in TraceValidator.REQUIRED_GENAI_ATTRIBUTES:
                if attr not in attributes:
                    errors.append(f"Span {span_id} missing required attribute: {attr}")

            # Validate expected system if provided
            if expected_system:
                actual_system = attributes.get("gen_ai.system")
                if actual_system != expected_system:
                    errors.append(
                        f"Span {span_id} has system '{actual_system}', expected '{expected_system}'"
                    )

        is_valid = len(errors) == 0
        logger.info(
            "GenAI trace validation complete",
            valid=is_valid,
            genai_span_count=len(genai_spans),
    ...
    @staticmethod
    def find_spans_by_operation(trace: Dict, operation: str) -> List[Dict]:
        """
        Find all spans with given operation name.
        ...
        for span in trace.get("spans", []):
            span_operation = span.get("attributes", {}).get("gen_ai.operation.name")
            if span_operation and operation in span_operation:
                matching_spans.append(span)

        logger.info(
            f"Found {len(matching_spans)} spans with operation containing '{operation}'"
        )
        return matching_spans

    @staticmethod
    def validate_ai_defense_events(trace: Dict) -> Dict:
        """
        Validate AI Defense security events in trace.

        Args:
            trace: Trace dictionary

        Returns:
            Validation result dictionary with 'valid' boolean and event details
        """
        security_spans = []
        events = []
        ...
        for span in trace.get("spans", []):
            attributes = span.get("attributes", {})
            if "gen_ai.security.event_id" in attributes:
                security_spans.append(span)
                events.append(
                    {
                        "event_id": attributes.get("gen_ai.security.event_id"),
                        "risk_level": attributes.get("gen_ai.security.risk_level"),
                        "span_id": span.get("spanId") or span.get("span_id"),
                    }
                )
        ...
        logger.info(
            "AI Defense events validated",
            security_span_count=len(security_spans),
    ...
    @staticmethod
    def is_genai_span(span: Dict) -> bool:
        """
        ...
        Returns:
            True if span has gen_ai.system attribute
        """
        return "gen_ai.system" in span.get("attributes", {})
