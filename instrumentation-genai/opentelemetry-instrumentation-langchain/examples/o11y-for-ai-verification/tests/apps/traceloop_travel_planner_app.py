...
**Purpose**:
Validates that the Traceloop translator correctly converts Traceloop SDK attributes
to OpenTelemetry GenAI semantic conventions without code changes.

**Key Validations**:
- ✅ Traceloop SDK @workflow and @task decorators function correctly
- ✅ Zero-code translator installation and activation
- ✅ Automatic attribute translation: traceloop.* → gen_ai.*
...
- ✅ DEEPEVAL_TELEMETRY_OPT_OUT environment variable support
- ✅ Multi-agent workflow orchestration with LangGraph
- ✅ Tool execution tracking and telemetry
...
# With DeepEval telemetry disabled (recommended)
DEEPEVAL_TELEMETRY_OPT_OUT=YES python traceloop_travel_planner_app.py
```

**Expected Telemetry**:
- Workflow spans with gen_ai.workflow.name attribute
- Task/agent spans with gen_ai.agent.name attribute
- Tool execution spans with proper parent-child relationships
- All traceloop.* attributes translated to gen_ai.* equivalents
"""
...
# Configure Splunk emitters for AI Details tab compatibility
# These must be set BEFORE any OpenTelemetry imports
...
def _configure_otlp_logging() -> None:
    ...
    # The SplunkEvaluationResultsEmitter uses EventLogger to emit evaluation events
    ...
    print("[INIT] EventLoggerProvider configured for evaluation events")
...
def print_configuration_summary():
    """Print configuration summary at startup (similar to direct_azure_openai_app_v2.py)"""
    print("\n" + "=" * 80)
    print("TRACELOOP TRAVEL PLANNER - Multi-Agent Workflow")
    print("=" * 80)
    print(f"\n🔧 Configuration:")
    print(f"  Service Name: {os.getenv('OTEL_SERVICE_NAME', 'travel-planner-traceloop')}")
    print(f"  Evaluators: {os.getenv('OTEL_INSTRUMENTATION_GENAI_EVALS_EVALUATORS', 'Not configured')}")
    print(f"  Sample Rate: {os.getenv('OTEL_INSTRUMENTATION_GENAI_EVALUATION_SAMPLE_RATE', '1.0')}")
    print(f"  Emitters: {os.getenv('OTEL_INSTRUMENTATION_GENAI_EMITTERS', 'default')}")
    print(f"  Content Capture: {os.getenv('OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT', 'false')}")
    print("=" * 80 + "\n")


@workflow(name="travel_planner_multi_agent")
def main() -> str:
    """Main workflow for multi-agent travel planning. Returns trace ID."""
    ...
    # Get and return trace ID for verification
    try:
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"  [ ] Trace ID visible in Splunk APM: {trace_id}")
        print(f"  [ ] AI Details tab shows evaluation metrics")
        print(f"  [ ] gen_ai.* attributes present (translated from traceloop.*)")
        print(f"  [ ] All 5 agents visible in trace hierarchy")
    except Exception as e:
