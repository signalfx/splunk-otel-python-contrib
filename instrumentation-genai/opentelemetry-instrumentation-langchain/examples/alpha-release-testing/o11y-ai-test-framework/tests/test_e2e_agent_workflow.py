...
class TestMultiAgentWorkflow:
    ...
    def test_langchain_agent_with_evaluation(
        ...
        prompt = DataGenerator.generate_prompts(count=1)[0]
        ...
        assert trace_available, f"Trace {trace_id} did not appear within {trace_wait_timeout}s"
        ...
        assert session_id_attr == test_session_id, \
        ...
        for llm_span in llm_spans:
            ...
            assert "gen_ai.operation.name" in span_attrs, "Missing gen_ai.operation.name"
    ...
    def test_agent_with_evaluation_metrics(
        ...
        eval_attrs = {k: v for k, v in attrs.items() if k.startswith("gen_ai.evaluation")}
