...
class TestTraceAPIValidation:
    ...
    def test_retrieve_trace_by_id(self, apm_client, trace_id_list):
        """
        ...
        # In real test: this trace would be generated by instrumented app
        # For now, we test the retrieval mechanism
        ...
        # Basic validation
        if trace_data:
            assert "spans" in trace_data or "resourceSpans" in trace_data, \
    ...
    def test_validate_span_attributes(self, apm_client):
        ...
        if trace_data and "spans" in trace_data:
            ...
            for span in spans:
                ...
                assert "spanId" in span or "span_id" in span, \
                    "Span missing ID"
                ...
                if "gen_ai.system" in attributes:
                    # Validate GenAI-specific attributes
                    result = SpanValidator.validate_span_attributes(
                        span=span,
                        required_attrs=["gen_ai.system", "gen_ai.operation.name"]
                    )
                    
                    assert result, f"Span {span.get('spanId')} missing required GenAI attributes"
    
    def test_validate_token_usage_attributes(self, apm_client):
        """
    ...
    def test_validate_span_hierarchy(self, apm_client):
        """
    ...
    def test_validate_span_timing(self, apm_client):
        """
    ...
    def test_find_spans_by_operation(self, apm_client):
        """
        ...
        if trace_data:
            ...
            assert isinstance(chat_spans, list), \
                "find_spans_by_operation should return a list"
    
    def test_validate_streaming_attributes(self, apm_client):
        """
    ...
    def test_validate_ai_defense_events(self, apm_client):
        """
    ...
    def test_batch_trace_retrieval(self, apm_client):
        """
        ...
        trace_ids = [DataGenerator.generate_trace_id() for _ in range(3)]
    ...
    def test_validate_attribute_types(self, apm_client):
        """
    ...
    def test_span_status_validation(self, apm_client):
        """
