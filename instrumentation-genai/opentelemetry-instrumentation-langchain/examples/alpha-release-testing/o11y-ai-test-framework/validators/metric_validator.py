...
class MetricValidator:
    ...
    TOKEN_USAGE_METRICS = ["gen_ai.client.token.usage", "gen_ai.server.token.usage"]

    COST_METRICS = ["gen_ai.client.operation.cost", "gen_ai.server.operation.cost"]

    @staticmethod
    def validate_evaluation_metrics(
        metrics: List[Dict], format_type: str = "auto"
    ) -> bool:
        """
        ...
        if format_type == "auto":
            # Auto-detect format
            has_new_format = any(
                m.get("name") == MetricValidator.NEW_FORMAT_METRIC for m in metrics
            )
            has_old_format = any(
                m.get("name") in MetricValidator.OLD_FORMAT_METRICS for m in metrics
            )
    ...
    def _validate_new_format(metrics: List[Dict]) -> bool:
        ...
        """
        eval_metrics = [
            m for m in metrics if m.get("name") == MetricValidator.NEW_FORMAT_METRIC
        ]
        ...
        for metric in eval_metrics:
            ...
            assert (
                eval_name is not None
            ), "Metric missing 'gen_ai.evaluation.name' attribute"
            assert (
                eval_name in MetricValidator.NEW_FORMAT_NAMES
    ...
    def validate_token_usage_metrics(metrics: List[Dict]) -> bool:
        ...
        """
        token_metrics = [
            m for m in metrics if m.get("name") in MetricValidator.TOKEN_USAGE_METRICS
        ]
        ...
        logger.info("Token usage metrics valid", count=len(token_metrics))
        return True
    ...
    def validate_cost_metrics(metrics: List[Dict]) -> bool:
        ...
        """
        cost_metrics = [
            m for m in metrics if m.get("name") in MetricValidator.COST_METRICS
        ]
        ...
        logger.info("Cost metrics valid", count=len(cost_metrics))
        return True
    ...
    def validate_metric_structure(metric: Dict) -> bool:
        ...
        assert (
            "timestamp" in metric or "time" in metric
    ...
    def compare_evaluation_scores(
        score1: float, score2: float, tolerance: float = 0.1
    ) -> bool:
        """
