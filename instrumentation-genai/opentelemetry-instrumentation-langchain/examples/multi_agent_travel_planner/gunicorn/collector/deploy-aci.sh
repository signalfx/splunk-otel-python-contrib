...
# Pinned to 0.123.0 — the sapm exporter was deprecated in 0.115.0 and removed
# in 0.147.0 (collector v1.12.0). If upgrading past 0.147.0, verify the
# otel-collector-config.yaml uses otlphttp/splunk instead of sapm for traces.
CONTAINER_IMAGE="${CONTAINER_IMAGE:-quay.io/signalfx/splunk-otel-collector:0.123.0}"
...
echo ""
echo " Update your App Service configuration:"
echo ""
echo "   az webapp config appsettings set \\"
echo "     --resource-group \"${RESOURCE_GROUP}\" \\"
echo "     --name \"\${APP_NAME}\" \\"
echo "     --settings OTEL_EXPORTER_OTLP_ENDPOINT=\"http://${COLLECTOR_IP}:4317\""
echo ""
echo " Health check:"
