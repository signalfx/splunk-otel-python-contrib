...
async def run_agent(user_query: str, manual: bool = True):
    ...
    # Export metrics every 2s so session-end metrics (mcp.server.session.duration)
    # are flushed before the MCP SDK's 2-second process-termination timeout expires.
    server_env.setdefault("OTEL_METRIC_EXPORT_INTERVAL", "2000")
