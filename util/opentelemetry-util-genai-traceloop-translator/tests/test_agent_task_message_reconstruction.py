...
class TestAgentMessageReconstruction:
    ...
    def test_agent_span_reconstructs_messages(self, setup_tracer_with_handler):
        ...
        assert "Paris" in output_msg.parts[0].content, "Output should mention Paris"
    ...
    def test_agent_span_has_genai_attributes(self, setup_tracer_with_handler):
        ...
        agent_span = next(
            (s for s in spans if s.name == "hotel_agent"),
            None
        )
...
class TestTaskMessageReconstruction:
    ...
    def test_task_span_reconstructs_messages(self, setup_tracer_with_handler):
        ...
        assert cached_input[0].parts[0].content == \
            "Search for flights from Seattle to Paris", "Input should match"
        assert "5 flights" in cached_output[0].parts[0].content, \
            "Output should mention flights"

    def test_task_span_without_messages_skips_reconstruction(self, setup_tracer_with_handler):
...
class TestLLMOperationMessageReconstruction:
    ...
    def test_chat_operation_reconstructs_messages(self, setup_tracer_with_handler):
    ...
    def test_completion_operation_reconstructs_messages(self, setup_tracer_with_handler):
...
class TestNonLLMSpanSkipsReconstruction:
    ...
    def test_workflow_span_skips_reconstruction(self, setup_tracer_with_handler):
    ...
    def test_unknown_span_skips_reconstruction(self, setup_tracer_with_handler):
...
class TestEdgeCases:
    ...
    def test_task_with_empty_messages(self, setup_tracer_with_handler):
        ...
        if span_id in processor._message_cache:
            cached_input, cached_output = processor._message_cache[span_id]
            assert cached_input == [], "Empty input should be cached as empty list"
            assert cached_output == [], "Empty output should be cached as empty list"

    def test_mixed_span_kinds(self, setup_tracer_with_handler):
        """Test different span kinds in same workflow."""
        tracer, exporter, provider, processor, _ = setup_tracer_with_handler

        input_data = json.dumps({"messages": [{"role": "user", "content": "Test"}]})
        output_data = json.dumps({"messages": [{"role": "assistant", "content": "OK"}]})
