"""Test message reconstruction for deeply nested Traceloop format.

This test module handles the complex nested format where Traceloop serializes
...
from opentelemetry.util.genai.processor.traceloop_message_reconstructor import (
    reconstruct_messages_from_traceloop,
)


class TestNestedTraceloopReconstruction:
    """Test reconstruction of deeply nested Traceloop message formats."""

    def test_reconstruct_nested_langchain_message(self):
        """Test reconstruction of nested LangChain message from Traceloop format."""
        # This is the actual format from Traceloop when serializing workflow inputs
        # The content field contains an escaped JSON string with LangChain message objects
        traceloop_input = json.dumps(
            [
                {
                    "role": "user",
                    "parts": [
        ...
        input_messages, _ = reconstruct_messages_from_traceloop(
    ...
    def test_normalize_deeply_nested_content(self):
        ...
        # Normalize
        normalized = normalize_traceloop_content(raw_input, "input")
    ...
    def test_extract_langchain_message_from_nested_json(self):
        ...
        # This is what we receive from Traceloop
        nested_content = {
            "args": [
                {
                    "messages": [
    ...
    def test_output_message_with_nested_parts(self):
        ...
        # Reconstruct messages
        _, output_messages = reconstruct_messages_from_traceloop(
