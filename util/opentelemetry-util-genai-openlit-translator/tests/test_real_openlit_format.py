"""Test with real openlit format from LangChain/LangGraph.

This test uses the actual format that openlit SDK produces, with:
...
class TestRealopenlitFormat:
    """Test with actual openlit SDK format."""

    def test_real_nested_input_format(self, setup_tracer):
        """Test with real openlit nested input format."""
        tracer, exporter, provider, processor = setup_tracer

        # Real openlit format with inputs.messages[]
        input_data = {
            "inputs": {
                "messages": [
                    {
        ...
        with tracer.start_as_current_span("ChatbotSummarizationAgent") as span:
            span.set_attribute("openlit.entity.input", json.dumps(input_data))
    ...
    def test_real_nested_output_format(self, setup_tracer):
        """Test with real openlit nested output format."""
        tracer, exporter, provider, processor = setup_tracer

        # Real openlit format with outputs.messages[]
        output_data = {
            "outputs": {
                "messages": [
                    {
        ...
        with tracer.start_as_current_span("ChatbotSummarizationAgent") as span:
            span.set_attribute(
                "openlit.entity.output", json.dumps(output_data)
            )
            span.set_attribute("llm.request.model", "gemini-1.5-flash")
            span_id = span.get_span_context().span_id
    ...
    def test_deepeval_extraction_with_real_format(self, setup_tracer):
        """Test that DeepEval can extract text from real openlit format."""
        tracer, exporter, provider, processor = setup_tracer
    ...
    def test_multiple_messages_in_real_format(self, setup_tracer):
        """Test with multiple messages in real openlit format."""
        tracer, exporter, provider, processor = setup_tracer
