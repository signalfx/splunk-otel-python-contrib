...
class TransformationRule:
    ...
    openlit_attributes: Dict[str, Any] = field(default_factory=dict)
...
class OpenlitSpanProcessor(SpanProcessor):
    ...
    def __init__(
        ...
        self.openlit_attributes = openlit_attributes or {}
        ...
        if self.rules:
            logging.getLogger(__name__).debug(
                "OpenlitSpanProcessor loaded %d transformation rules (explicit=%d env=%d)",
                len(self.rules),
                len(rules or []),
                len(env_rules),
            )
    ...
    def _process_span_translation(self, span: ReadableSpan) -> Optional[Any]:
        ...
        # NOTE: _openlit_processed is set by mutation, _openlit_translated is set by translation
        ...
        if applied_rule is not None:
            ...
            extra_tl_attrs = {
                **applied_rule.openlit_attributes,
                **sentinel,
            }
        else:
            attr_tx = self.attribute_transformations
            name_tx = self.name_transformations
            extra_tl_attrs = {**self.openlit_attributes, **sentinel}
        ...
        invocation = self._build_invocation(
            ...
            openlit_attributes=extra_tl_attrs,
        )
        ...
        invocation.attributes.setdefault("_openlit_processed", True)
    ...
    def _reconstruct_and_set_messages(
        ...
        """
        Reconstruct messages from openlit format and set them as gen_ai.* attributes.
    ...
    def _mutate_span_if_needed(self, span: ReadableSpan) -> None:
        ...
        # Skip if already processed (original openlit spans)
        if span.attributes and "_openlit_processed" in span.attributes:
            return
        ...
        if applied_rule is not None:
            attr_tx = applied_rule.attribute_transformations
            name_tx = applied_rule.name_transformations
            extra_attrs = applied_rule.openlit_attributes
        else:
            attr_tx = self.attribute_transformations
            name_tx = self.name_transformations
            extra_attrs = self.openlit_attributes
        ...
        if should_mutate and (attr_tx or extra_attrs):
            try:
                ...
                if hasattr(span, "_attributes"):
                    ...
                    # Mark as processed
                    mutated["_openlit_processed"] = True
    ...
    def _convert_langchain_to_genai_messages(
        ...
        for lc_msg in langchain_messages:
            try:
                ...
                # Basically only use the "content" of the incoming openlit entity input/output
                if (
    ...
    def _build_invocation(
        ...
        *,
        ...
        # Read from _attributes (the live/mutated dict), NOT from .attributes
        ...
        # If so, skip re-applying transformations since they were already applied during mutation
        already_mutated = base_attrs.get("_openlit_processed", False)
        ...
        if openlit_attributes:
            ...
            if not already_mutated:
                transformed_tl_attrs = self._apply_attribute_transformations(
                    openlit_attributes.copy(), attribute_transformations
                )
                base_attrs.update(transformed_tl_attrs)
            else:
                # Just add the extra attributes without transformation
                base_attrs.update(openlit_attributes)
        ...
        # For openlit task/workflow spans without model info, preserve original span name
