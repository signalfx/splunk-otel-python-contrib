...
This module supports two evaluation modes controlled by an environment variable:

    OTEL_INSTRUMENTATION_GENAI_EVALS_DEEPEVAL_MODE

Values:
    - "deepeval" (default): Uses the deepeval package with per-metric LLM calls.
      Requires the deepeval package to be installed.
    - "batched": Uses inline rubrics with a single batched LLM call for all metrics.
      Does NOT require the deepeval package. Lighter weight but may differ in results.

When mode is "batched", the DeepevalBatchedEvaluator from deepeval_batched.py is used.
"""
...
# Environment variable to select the evaluation mode
OTEL_INSTRUMENTATION_GENAI_EVALS_DEEPEVAL_MODE = (
    "OTEL_INSTRUMENTATION_GENAI_EVALS_DEEPEVAL_MODE"
)

# Valid mode values
_MODE_DEEPEVAL = "deepeval"
_MODE_BATCHED = "batched"


def _get_deepeval_mode() -> str:
    """Get the configured deepeval mode from environment variable."""
    mode = (
        os.getenv(OTEL_INSTRUMENTATION_GENAI_EVALS_DEEPEVAL_MODE, "")
        .strip()
        .lower()
    )
    if mode == _MODE_BATCHED:
        return _MODE_BATCHED
    return _MODE_DEEPEVAL  # default
...
def _factory(
    ...
    """Factory function that returns the appropriate evaluator based on mode.

    When OTEL_INSTRUMENTATION_GENAI_EVALS_DEEPEVAL_MODE=batched, returns
    DeepevalBatchedEvaluator. Otherwise returns the standard DeepevalEvaluator.
    """
    mode = _get_deepeval_mode()
    if mode == _MODE_BATCHED:
        # Import here to avoid circular imports and keep batched optional
...
def register() -> None:
    """Register the deepeval evaluator.

    The evaluator mode is controlled by OTEL_INSTRUMENTATION_GENAI_EVALS_DEEPEVAL_MODE:
    - "deepeval" (default): Uses the deepeval package with per-metric LLM calls.
    - "batched": Uses inline rubrics with a single batched LLM call.
    """
    register_evaluator(
        "deepeval",
