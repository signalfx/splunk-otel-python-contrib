...
Examples:
    ...
    python invocation_example.py eval 5      # Run first 5 test cases
...
def main():
    ...
    if len(sys.argv) > 1:
        invocation_type = sys.argv[1].lower()
    else:
        invocation_type = "all"

    # Parse optional limit for eval mode
    eval_limit = None
    if invocation_type == "eval" and len(sys.argv) > 2:
        try:
            eval_limit = int(sys.argv[2])
        except ValueError:
            print(f"Error: Invalid limit '{sys.argv[2]}' - must be a number")
            sys.exit(1)
    ...
    if invocation_type == "eval":
        ...
        if not api_key:
            # Try to read from ~/.cr/.cr.openai
            try:
                cr_file = Path.home() / ".cr" / ".cr.openai"
                if cr_file.exists():
